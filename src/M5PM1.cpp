...
m5pm1_err_t M5PM1::begin(TwoWire *wire, uint8_t addr, int8_t sda, int8_t scl, uint32_t speed) {
    ...
    m5pm1_i2c_speed_t targetSpeed = (_requestedSpeed == M5PM1_I2C_FREQ_400K)
        ? M5PM1_I2C_SPEED_400K : M5PM1_I2C_SPEED_100K;
    ...
    M5PM1_LOG_I(TAG, "M5PM1 initialized at address 0x%02X (I2C: %lu Hz)", _addr,
...
m5pm1_err_t M5PM1::begin(i2c_port_t port, uint8_t addr, int sda, int scl, uint32_t speed) {
    ...
    ret = i2c_master_bus_add_device(_i2c_master_bus, &dev_config, &_i2c_master_dev);
    if (ret != ESP_OK) {
        M5PM1_LOG_E(TAG, "Failed to switch device to %lu Hz: %s",
                    (unsigned long)_requestedSpeed, esp_err_to_name(ret));
        i2c_del_master_bus(_i2c_master_bus);
        _i2c_master_bus = nullptr;
        _initialized = false;
        return M5PM1_ERR_I2C_CONFIG;
...
void M5PM1::pinMode(uint8_t pin, uint8_t mode) { pinModeWithRes(pin, mode, nullptr); }
void M5PM1::digitalWrite(uint8_t pin, uint8_t value) { digitalWriteWithRes(pin, value, nullptr); }
int M5PM1::digitalRead(uint8_t pin) { return digitalReadWithRes(pin, nullptr); }
...
m5pm1_err_t M5PM1::gpioSet(m5pm1_gpio_num_t pin, m5pm1_gpio_mode_t mode,
...
m5pm1_err_t M5PM1::setPwmFrequency(uint16_t frequency) {
    ...
    if (actualFreq != frequency) {
        M5PM1_LOG_E(TAG, "PWM frequency verification failed: expected=%d, actual=%d",
...
m5pm1_err_t M5PM1::setPwmDuty(m5pm1_pwm_channel_t channel, uint8_t duty,
...
m5pm1_err_t M5PM1::getPwmDuty(m5pm1_pwm_channel_t channel, uint8_t* duty,
                             bool* polarity, bool* enable) {
...
m5pm1_err_t M5PM1::setPwmDuty12bit(m5pm1_pwm_channel_t channel, uint16_t duty12,
    ...
    if (actualDuty12 != duty12 || actualPolarity != polarity || actualEnable != enable) {
        M5PM1_LOG_E(TAG, "PWM channel %d verification failed: duty=%d/%d, pol=%d/%d, en=%d/%d",
    ...
    M5PM1_LOG_I(TAG, "PWM channel %d set and verified: duty=%d, pol=%d, en=%d",
...
m5pm1_err_t M5PM1::setLedCount(uint8_t count) {
    ...
    if (!countMatch) {
        M5PM1_LOG_E(TAG, "LED count verification failed: expected=%d, actual=%d",
                    count, actualCfg & M5PM1_NEO_CFG_COUNT_MASK);
        return M5PM1_FAIL;
    }
...
m5pm1_err_t M5PM1::setLedColor(uint8_t index, uint8_t r, uint8_t g, uint8_t b) {
    ...
    if (actualRgb565 != rgb565) {
        M5PM1_LOG_E(TAG, "LED color verification failed for index %d: expected=0x%04X, actual=0x%04X",
...
m5pm1_err_t M5PM1::setLeds(const m5pm1_rgb_t* colors, uint8_t arraySize,
                           uint8_t count, bool autoRefresh) {
    if (!_initialized) {
    ...
    for (uint8_t i = 0; i < count; i++) {
        uint16_t rgb565 = ((uint16_t)(colors[i].r & 0xF8) << 8)
...
m5pm1_err_t M5PM1::setAw8737aPulse(m5pm1_gpio_num_t pin, m5pm1_aw8737a_pulse_t num,
                                  m5pm1_aw8737a_refresh_t refresh) {
    if (!_initialized) {
        M5PM1_LOG_E(TAG, "Device not initialized");
        return M5PM1_ERR_NOT_INIT;
    }
    ...
    if (actualValue != expectedValue) {
        M5PM1_LOG_E(TAG, "AW8737A pulse verification failed: expected=0x%02X, actual=0x%02X",
                    expectedValue, actualValue);
        return M5PM1_FAIL;
    }

    M5PM1_LOG_I(TAG, "AW8737A pulse set and verified: pin=%d, num=%d, refresh=%d (reg=0x%02X)",
                pin, num, refresh, regValue);
    return M5PM1_OK;
}
...
m5pm1_err_t M5PM1::writeRtcRAM(uint8_t offset, const uint8_t* data, uint8_t len) {
    if (data == nullptr || offset >= M5PM1_RTC_RAM_SIZE || len == 0 ||
        (offset + len) > M5PM1_RTC_RAM_SIZE) {
        return M5PM1_ERR_INVALID_ARG;
    }
    if (!_initialized) {
        return M5PM1_ERR_NOT_INIT;
    ...
    for (uint8_t i = 0; i < len; i++) {
        if (actualData[i] != data[i]) {
            M5PM1_LOG_E(TAG, "RTC_RAM verification failed at offset %d: expected=0x%02X, actual=0x%02X",
...
// ============================
 ...
 m5pm1_err_t M5PM1::setI2cConfig(uint8_t sleepTime, m5pm1_i2c_speed_t speed) {
    ...
    if (actualSleep != expectedSleep ||
        actualSpeed400k != (speed == M5PM1_I2C_SPEED_400K)) {
    ...
    M5PM1_LOG_I(TAG, "I2C config set and verified: sleep=%u, speed=%s",
                sleepTime, speed == M5PM1_I2C_SPEED_400K ? "400K" : "100K");
    return M5PM1_OK;
}
...
m5pm1_err_t M5PM1::setI2cSleepTime(uint8_t seconds) {
    ...
    if (actualSleep != seconds) {
        M5PM1_LOG_E(TAG, "I2C_CFG sleep time verification failed: expected=%u, actual=%u",
                    seconds, actualSleep);
        return M5PM1_FAIL;
    }
...
m5pm1_err_t M5PM1::switchI2cSpeed(m5pm1_i2c_speed_t speed) {
    ...
    uint32_t originalFreq = (i2cCfg & M5PM1_I2C_CFG_SPEED_400K)
        ? M5PM1_I2C_FREQ_400K : M5PM1_I2C_FREQ_100K;
    ...
    switch (_i2cDriverType) {
        ...
        case M5PM1_I2C_DRIVER_MASTER: {
            if (_i2c_master_dev != nullptr) {
                ...
                ret = i2c_master_bus_add_device(_i2c_master_bus, &dev_config, &_i2c_master_dev);
                if (ret != ESP_OK) {
                    M5PM1_LOG_E(TAG, "Failed to add I2C device at %lu Hz: %s",
                                (unsigned long)targetFreq, esp_err_to_name(ret));
    ...
    uint8_t id = 0;
    if (!_readReg(M5PM1_REG_DEVICE_ID, &id)) {
        M5PM1_LOG_E(TAG, "Communication failed after switching to %lu Hz, reverting",
...
bool M5PM1::isAutoWakeEnabled() const {
 ...
 m5pm1_err_t M5PM1::sendWakeSignal() {
    ...
    switch (_i2cDriverType) {
        case M5PM1_I2C_DRIVER_SELF_CREATED:
        case M5PM1_I2C_DRIVER_MASTER:
            return M5PM1_I2C_MASTER_SEND_WAKE(_i2c_master_bus, _addr) == ESP_OK
                ? M5PM1_OK : M5PM1_ERR_I2C_COMM;
        case M5PM1_I2C_DRIVER_BUS:
            return M5PM1_I2C_SEND_WAKE(_i2c_device, M5PM1_REG_HW_REV) == ESP_OK
                ? M5PM1_OK : M5PM1_ERR_I2C_COMM;
        default:
            return M5PM1_ERR_INTERNAL;
    }
...
m5pm1_snapshot_verify_t M5PM1::verifySnapshot() {
    ...
    if (_cacheValid) {
        ...
        if (_readReg(M5PM1_REG_GPIO_MODE, &actualMode) &&
            _readReg(M5PM1_REG_GPIO_OUT, &actualOut)) {
    ...
    if (_adcStateValid) {
        ...
        uint16_t value = 0;
        if (_readReg(M5PM1_REG_ADC_CTRL, &ctrl) &&
            _readReg16(M5PM1_REG_ADC_RES_L, &value)) {
            uint8_t actualChannel = (ctrl >> 1) & 0x07;
            bool actualBusy = (ctrl & 0x01) != 0;

            if (actualChannel != _adcState.channel ||
                actualBusy != _adcState.busy ||
                value != _adcState.lastValue) {
    ...
    if (_powerConfigValid) {
        ...
        if (_readReg(M5PM1_REG_PWR_CFG, &actualPwr) &&
            _readReg(M5PM1_REG_HOLD_CFG, &actualHold)) {
    ...
    if (_btnConfigValid) {
        ...
        if (_readReg(M5PM1_REG_BTN_CFG_1, &actualCfg1) &&
            _readReg(M5PM1_REG_BTN_CFG_2, &actualCfg2)) {
    ...
    if (_irqMaskValid) {
        ...
        if (_readReg(M5PM1_REG_IRQ_MASK1, &actualMask1) &&
            _readReg(M5PM1_REG_IRQ_MASK2, &actualMask2) &&
    ...
    switch (configType) {
        ...
        case M5PM1_CONFIG_GPIO_OUTPUT:
            ...
            if (_hasActiveAdc(pin)) {
                snprintf(result.error_msg, sizeof(result.error_msg),
                         "Pin %d is used for ADC", pin);
                return result;
            }
            if (_hasActiveNeo(pin)) {
                snprintf(result.error_msg, sizeof(result.error_msg),
                         "Pin %d is used for LED_EN", pin);
                return result;
            }
            if (_hasActiveIrq(pin)) {
                snprintf(result.error_msg, sizeof(result.error_msg),
                         "Pin %d has IRQ enabled", pin);
                return result;
            }
            if (_hasActiveWake(pin)) {
                snprintf(result.error_msg, sizeof(result.error_msg),
                         "Pin %d has WAKE enabled", pin);
                return result;
            }
            break;
        ...
        case M5PM1_CONFIG_NEOPIXEL:
            if (!_isNeoPin(pin)) {
                snprintf(result.error_msg, sizeof(result.error_msg),
                         "NeoPixel only supported on GPIO0");
                return result;
            }
            if (_hasActiveIrq(pin)) {
                snprintf(result.error_msg, sizeof(result.error_msg),
                         "GPIO0 has IRQ enabled");
                return result;
            }
            if (_hasActiveWake(pin)) {
                snprintf(result.error_msg, sizeof(result.error_msg),
                         "GPIO0 has WAKE enabled");
                return result;
            }
            break;
